...
- **开关与配置**：`LANGFUSE_ENABLED`（默认 false）+ `LANGFUSE_HOST` / `LANGFUSE_PUBLIC_KEY` / `LANGFUSE_SECRET_KEY`（密钥只经 .env/环境注入，禁止提交与落日志）。关闭时 langfuse 模块零导入、零开销；开启但密钥缺失 → 启动期 WARN 降级为关闭（可观测故障不阻断业务）。
