...
retrieval_planner_agent（检索计划，四类策略多选）
  ...
  → hybrid_retriever_node（汇总全部查询去重/排除已检索/截断 ≤8，asyncio.gather 并发，
    ...
    推送 plan 事件=本轮全部检索查询）
  → evidence_ranking_node（dedup 三级键合并 matched_queries → RRF 预截断 32 →
...
质量优化还保持以下局部规则：`retrieval_planner` 的 `target_evidence` 只是检索提示，
`evidence_grader` 不得把证据上下文之外的法条、司法解释或扩展制度当成必需证据；
当原问题明确点名多个法源时，`evidence_ranking_node` 在已完成的同一次重排结果中为每个
被点名法源保留最高分证据，再补齐统一 top-10，不增加节点或边，也不增加一次 Reranker 请求。
若问题未点名多个法源但同一法源连续占据重排结果前两位，则保留该法源的一致性上下文，
避免低相关法源占据引用列表；这是轻量来源选择启发式，不改变多法源显式覆盖规则。
证据不足状态下的回答会明确区分“材料能证明的范围”和“材料不能判断的结论”，避免用无关
法条推断犯罪、刑期、金额等问题。
