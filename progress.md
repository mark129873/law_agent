...
- 标准验证路径：`cd backend && uv run pytest tests -q -rs`（当前 242 个自动化测试；Docker/Milvus 可用时本轮 242 passed、1 warning，5 个 Milvus 集成测试均实际执行）；启动后 `curl http://127.0.0.1:8000/api/health`
...
- 当前 blocker：无代码 blocker；真实 Tavily Remote MCP smoke 与真实 Milvus/GLM RAG E2E 已通过；本轮 CUA 浏览器 inventory helper 不可用，前端以 `npm run build` 和 API/组件契约验证，未完成可见浏览器回归
