...
QUERY_ROUTER_SYSTEM_PROMPT = (
    ...
    "\n"
    "# 判定规则\n"
    "1. 只有用户明确要求联网、上网、网页、搜索最新/实时信息时才判 web_request；\n"
    "2. 仅出现『现行法律』『是否规定』『法律规定』等法律表述，不等于要求联网，"
    "应判 legal_question → local_rag；\n"
    "3. 拿不准是否需要法律依据时，倾向 legal_question——宁可多检索，不可漏依据；\n"
    "4. 提到具体法律条文、案例、赔偿计算、诉讼程序的，一律判 legal_question；\n"
    "5. normalized_query 保留原意：补全省略的主语与法律要素，去掉口语冗余，"
    "不改变事实内容；\n"
